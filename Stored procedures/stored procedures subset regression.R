...
model_formula <- function(id=8,object=bestsubset,outcome="collection"){
  # get models data
...
#check for max adjusted R^2 and minimum prediction errors -RSS, cp, BIC- (Subset Selection)
variable_name <- summary(model-name)
data.frame(
  Adj.R2 = which.max(variable_name$adjr2),
  CP = which.min(variable_name.sum$cp),
  BIC = which.min(variable_name.sum$bic),
  RSS=which.min(variable_name$rss)
)
